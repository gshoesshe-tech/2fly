...
function shortId(s){const t=String(s||"");return t.length<=8?t:t.slice(0,4)+"…"+t.slice(-4)}
...
function title(path){return {"#/orders":"Orders",
    "#/order":"Order Details","#/new-order":"New Order","#/inventory":"Inventory","#/commissions":"Commissions","#/products":"Products","#/finance":"Finance","#/dashboard":"Dashboard"}[path]||"2FLY Internal";}
...
async function orderDetailsPage(role, orderId){
  if(!orderId){
    return `<div class="card"><div class="error">Missing order id. Go back to <a class="link" href="#/orders">Orders</a>.</div></div>`;
  }
  const isOwnerRole = role==="owner" || role==="admin";

  const { data: o, error: oe } = await supabase.from("orders").select("*").eq("id", orderId).maybeSingle();
  if(oe) return `<div class="card"><div class="error">${esc(oe.message||"Failed to load order")}</div></div>`;
  if(!o) return `<div class="card"><div class="error">Order not found.</div></div>`;

  const { data: items, error: ie } = await supabase
    .from("order_items")
    .select("id,sku,qty,category_at_time,unit_cost_at_time,sell_price_at_time,products(name)")
    .eq("order_id", orderId)
    .order("created_at", { ascending: true });
  if(ie) return `<div class="card"><div class="error">${esc(ie.message||"Failed to load items")}</div></div>`;

  let itemsTotal = 0, cogsTotal = 0;
  for(const it of (items||[])){
    const rev = (Number(it.sell_price_at_time)||0) * (Number(it.qty)||0);
    const cogs = (Number(it.unit_cost_at_time)||0) * (Number(it.qty)||0);
    itemsTotal += rev; cogsTotal += cogs;
  }
  const profitTotal = itemsTotal - cogsTotal;

  const statusOptions = ["pending","paid","packed","shipped","completed","cancelled"]
    .map(s=>`<option value="${s}" ${o.status===s?"selected":""}>${s.toUpperCase()}</option>`).join("");

  const orderInfo = `
    <div class="card">
      <div class="row">
        <div>
          <div class="h2">${esc(o.order_code||"Order")}</div>
          <div class="small mono">ID: ${esc(o.id)}</div>
          <div class="small">Type: <span class="pill">${esc((o.order_type||"").toUpperCase())}</span></div>
        </div>
        <div class="row" style="gap:10px">
          <select id="odStatus" class="input">${statusOptions}</select>
          <button class="btn primary" id="odSaveStatus" data-id="${esc(o.id)}">Save status</button>
        </div>
      </div>

      <div class="grid" style="margin-top:12px">
        <div class="panel" style="padding:14px; border-radius:18px">
          <div class="label">Customer</div>
          <div class="big">${esc(o.customer_name||"-")}</div>
          <div class="small">Phone: <span class="mono">${esc(o.phone_number||"-")}</span></div>
          <div class="small">FB: ${o.profile_link?`<a class="link" href="${esc(o.profile_link)}" target="_blank" rel="noreferrer">Open profile</a>`:"-"}</div>
        </div>

        <div class="panel" style="padding:14px; border-radius:18px">
          <div class="label">Shipping / Notes</div>
          <div class="small">Region: <span class="mono">${esc(o.region||"-")}</span></div>
          <div class="small">Shipping paid: <b>${fmtMoney(o.shipping_paid||0)}</b></div>
          <div class="small">Courier cost: <span class="mono">${fmtMoney(o.courier_cost||0)}</span></div>
          <div class="small">Notes: ${esc(o.notes||"-")}</div>
        </div>
      </div>
    </div>
  `;
  ...
  const rows = (items||[]).map((it)=> {
    ...
    const price = Number(it.sell_price_at_time)||0;
    const cost = Number(it.unit_cost_at_time)||0;
    const qty = Number(it.qty)||0;
    const total = price*qty;
    const cogs = cost*qty;
    const prof = total-cogs;

    return `
      <tr data-item-row="1" data-id="${esc(it.id)}" data-sku="${esc(it.sku)}">
        <td class="mono">${esc(it.sku)}</td>
        <td>${esc(name)}</td>
        <td class="mono">${qty}</td>
        <td class="mono">${fmtMoney(price)}</td>
        <td class="mono"><b>${fmtMoney(total)}</b></td>
        ${isOwnerRole?`<td class="mono">${fmtMoney(cost)}</td><td class="mono">${fmtMoney(prof)}</td>`:""}
        ${isOwnerRole?`<td class="actions"><button class="btn small danger odRemoveItem" data-id="${esc(it.id)}">Remove</button></td>`:""}
      </tr>
    `;
  }).join("");

  const headerCols = `
    <th>SKU</th><th>Item</th><th>Qty</th><th>Sell Price</th><th>Total</th>
    ${isOwnerRole?`<th>Unit cost</th><th>Profit</th>`:""}
    ${isOwnerRole?`<th>Actions</th>`:""}
  `;

  const totalsRow = `
    <div class="row space wrap mt">
      <div class="small">Items total: <b>${fmtMoney(itemsTotal)}</b></div>
      ${isOwnerRole?`<div class="small">COGS: <b>${fmtMoney(cogsTotal)}</b> • Items profit: <b>${fmtMoney(profitTotal)}</b></div>`:""}
    </div>
  `;

  // Owner-only item editor (replace items)
  let editor = "";
  if(isOwnerRole){
    const prods = await loadProducts();
    const options = productOptions(prods);
    editor = `
      <div class="card mt">
        <div class="row">
          <div>
            <div class="h2">Edit items</div>
            <div class="small">This will replace the order's items and re-calculate inventory & profit.</div>
          </div>
          <button class="btn" id="odReload">Reload</button>
        </div>

        <div class="row" style="gap:10px; margin-top:12px">
          <select id="odAddSku" class="input">${options}</select>
          <input id="odAddQty" class="input" type="number" min="1" value="1" style="max-width:120px"/>
          <button class="btn" id="odAddBtn">Add</button>
        </div>

        <div class="small mt">Current items (edit qty):</div>
        <div id="odEditList" style="margin-top:12px">
          ${(items||[]).map((it)=>`
            <div class="row gap wrap odEditRow" data-sku="${esc(it.sku)}">
              <span class="mono" style="min-width:90px">${esc(it.sku)}</span>
              <input class="input odEditQty" type="number" min="1" value="${Number(it.qty)||1}" style="max-width:120px"/>
              <button class="btn small danger odEditRemove">Remove</button>
            </div>
          `).join("") || `<div class="small">No items.</div>`}
        </div>

        <div class="row" style="gap:10px; margin-top:12px">
          <button class="btn primary" id="odSaveItems" data-id="${esc(o.id)}">Save items</button>
          <div class="small" id="odEditMsg"></div>
        </div>
      </div>
    `;
  }
    ...
    <div class="card mt">
      ...
      <div style="margin-top:12px">
        <table class="table">
          ...
          <tbody>${rows || `<tr><td colspan="${isOwnerRole?8:5}" class="small">No items.</td></tr>`}</tbody>
        </table>
      </div>
      ${totalsRow}
    </div>
...
function bindOrderDetails(){
  const orderId = qs().query.id || qs().query.order_id || "";
  const $status = document.getElementById("odStatus");
  const $save = document.getElementById("odSaveStatus");
  const $reload = document.getElementById("odReload");

  $save?.addEventListener("click", async ()=>{
    const status = ($status?.value||"").toLowerCase();
    $save.disabled = true;
    try{
      const { error } = await supabase.rpc("staff_update_status", { p_order_id: orderId, p_status: status });
      if(error) throw error;
  ...
  $reload?.addEventListener("click", ()=> location.hash = `#/order?id=${orderId}`);

  // Owner editor bindings
  const $addBtn = document.getElementById("odAddBtn");
  const $addSku = document.getElementById("odAddSku");
  const $addQty = document.getElementById("odAddQty");
  const $list = document.getElementById("odEditList");
  const $saveItems = document.getElementById("odSaveItems");
  const $msg = document.getElementById("odEditMsg");

  function getEditorItems(){
    const rows = Array.from($list?.querySelectorAll(".odEditRow")||[]);
    const items = [];
    for(const r of rows){
      const sku = r.getAttribute("data-sku");
      const qty = Number(r.querySelector(".odEditQty")?.value||0);
  ...
  function renderEditorItems(items){
    ...
    $list.querySelectorAll(".odEditRemove").forEach(btn=>{
      btn.addEventListener("click", ()=>{
        btn.closest(".odEditRow")?.remove();
      });
    });
  }
  ...
  $addBtn?.addEventListener("click", ()=>{
    ...
    renderEditorItems(items);
    $addQty.value = "1";
  });

  // attach remove to initial
  $list?.querySelectorAll(".odEditRemove").forEach(btn=>{
    btn.addEventListener("click", ()=> btn.closest(".odEditRow")?.remove());
  });
  ...
  $saveItems?.addEventListener("click", async ()=>{
    ...
    try{
      const { error } = await supabase.rpc("owner_replace_order_items", { p_order_id: orderId, p_items: items });
      if(error) throw error;
      if($msg) $msg.textContent = "Saved!";
      location.hash = `#/order?id=${orderId}`;
    }catch(e){
      if($msg) $msg.textContent = "";
      alert(e.message || "Failed to save items");
    }finally{
      $saveItems.disabled = false;
    }
  });

  // Remove buttons on view table (owner)
  document.querySelectorAll(".odRemoveItem").forEach(btn=>{
    btn.addEventListener("click", ()=>{
      const row = btn.closest("tr");
      const sku = row?.getAttribute("data-sku");
      const qty = Number(row?.querySelectorAll("td")[2]?.textContent||0);
      if(!$list || !sku) return;
      const items = getEditorItems().filter(x=>x.sku!==sku);
      // if we didn't have an editor list yet, init from existing rows
      if(items.length===0){
        // Build from current editor rows (if empty) - keep empty.
      }
      renderEditorItems(items);
      alert("Item removed in editor. Click 'Save items' to apply.");
    });
  });
}
...
async function route(session,profile){
  ...
  else if(path==="#/order"){const id=qs().query.id||qs().query.order_id||""; content=await orderDetailsPage(role,id); rightAction=`<a class="btn" href="#/orders">← Back to Orders</a>`;}
  ...
  if(qs().path==="#/order") bindOrderDetails();
